...
reload blocker is not the active cause of the example tenant human-root thread drop
tracked in `0020-2026-05-05-example-slack-thread-participation-gap.md`, and the
Socket Mode transport liveness gap is now resolved in
`0017-2026-05-04-example-slack-socket-stale-ingress.md`.
...
example tenant Slack access changes were written correctly to the user-scoped
`openclaw.json`, but the gateway did not apply them while handling live traffic.
authorized-user-b messaged `#ask-lei` and received no response because the
Slack channel reload was deferred behind task bookkeeping that had become stale.

The file-backed config included the expected example tenant allowlist and channel users:

```text
user-id-redacted authorized-user-a
user-id-redacted authorized-user-b
user-id-redacted authorized-user-c
```
...
`cli` task records for `odollo-example` monitor-dispatch work. Cancelling those
...
openclaw tasks cancel RUN_ID_REDACTED
openclaw tasks cancel RUN_ID_REDACTED
openclaw status --deep
```

After restart, Slack startup was slow but eventually applied the expected
example-tenant state:

```text
slack startup step "socket start" completed in 80570ms
slack channels resolved: channel-id-redacted->ask-lei
slack users resolved: user-id-redacted->authorized-user-a, user-id-redacted->authorized-user-b, user-id-redacted->authorized-user-c
slack channel users resolved: user-id-redacted->authorized-user-a, user-id-redacted->authorized-user-b, user-id-redacted->authorized-user-c
```
...
Slack OK ok (default:default, example:example)
```
...
- Add tenant/account labels to Slack Socket Mode logs so default-vs-example tenant
...
## Follow-up Observation: authorized-user-c Identity Correction

On 2026-04-28, the example tenant `#ask-lei` allowlist needed another live identity
correction: authorized-user-c's real sender id in the example tenant tenant was observed as
`user-id-redacted`, while the durable config still allowed `user-id-redacted`.

The file-backed config and workspace policy were corrected in the user-scoped
runtime home, and a direct Slack Web API threaded response to authorized-user-c succeeded.
However, the gateway again logged a deferred channel reload after the config
change:

```text
[reload] config change detected; evaluating reload (channels.slack.accounts.example.allowFrom, channels.slack.accounts.example.channels.channel-id-redacted.users)
...
`odollo-example` CLI monitor-dispatch runs. Because the records had no task id,
...
`odollo-example` CLI monitor-dispatch runs, but the same failure class would
...
```text
/home/user/workspace.local/odollo/doc/dev/notes/openclaw-task-ledger-handoff-2026-05-01.md
```
...
- The two stale example tenant `odollo-example` monitor-dispatch `running` tasks were
...
- This closes the current cancellable-id example tenant stale-task incident.
