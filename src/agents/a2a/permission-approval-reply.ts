...
function buildPatchSummary(request: SessionAccessPermissionRequest): string {
  if (request.reason === "agent_to_agent_disabled") {
    return (
      "Set `tools.agentToAgent.enabled=true`. This enables the global A2A gate; " +
      "the allow list governs membership, and an empty allow list permits every agent."
    );
  }
  if (request.reason === "agent_to_agent_allow") {
    ...
    if (missing.length === 0) {
      return (
        "Update `tools.agentToAgent.allow`. This is membership-wide, not pair-scoped: " +
        "each admitted agent can access every other agent matched by the allow list."
      );
    }
    const additions =
      missing.length === 1
        ? `Add \`${missing[0]}\` to \`tools.agentToAgent.allow\`.`
        : `Add ${missing.map((agentId) => `\`${agentId}\``).join(", ")} to \`tools.agentToAgent.allow\`.`;
    return `${additions} This is membership-wide, not pair-scoped: each added agent can access every other agent matched by the allow list.`;
  }
  return (
    "Set `tools.sessions.visibility=all`. This globally exposes all agents' sessions to session tools; " +
    "the A2A policy still gates cross-agent access."
  );
}
...
function buildPendingApprovalText(
  ...
  lines.push("Approve the config change described above, then retry the request.");
