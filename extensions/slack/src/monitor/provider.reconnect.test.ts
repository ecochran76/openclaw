...
import { createSlackSocketUnhandledRejectionGuard } from "./provider.js";
import {
...
describe("slack socket reconnect helpers", () => {
  ...
  it("suppresses only the exact Slack socket fault object once", () => {
    let now = 1_711_406_400_000;
    const guard = createSlackSocketUnhandledRejectionGuard(() => now);
    const socketFault = new Error("ECONNRESET");

    guard.noteFault(socketFault);

    expect(guard.consumeFault(new Error("ECONNRESET"))).toBeUndefined();
    expect(guard.consumeFault(socketFault)).toBe("ECONNRESET");
    expect(guard.consumeFault(socketFault)).toBeUndefined();

    const expiredFault = new Error("socket hang up");
    guard.noteFault(expiredFault);
    now += 15_001;
    expect(guard.consumeFault(expiredFault)).toBeUndefined();
  });
  ...
  it("marks an errored active socket reconnecting instead of healthy", () => {
    ...
    installSlackSocketModeStatusObserver(receiver, setStatus, () => status, {
  ...
  it("preserves multi-socket aggregation with setStatus-only publishing", () => {
    const primary = new FakeEmitter() as FakeEmitter & {
      websocket?: { isActive: () => boolean };
    };
    const secondary = new FakeEmitter() as FakeEmitter & {
    ...
    installSlackSocketModeStatusObserver({ client: primary }, setStatus, undefined, {
      connectionId: "primary",
      activeProbeIntervalMs: 0,
    });
    installSlackSocketModeStatusObserver({ client: secondary }, setStatus, undefined, {
      connectionId: "socket-2",
      activeProbeIntervalMs: 0,
    });
  ...
  it("does not resurrect a disposed socket from stale status readback", () => {
    const primary = new FakeEmitter() as FakeEmitter & {
      websocket?: { isActive: () => boolean };
    };
    const secondary = new FakeEmitter() as FakeEmitter & {
    ...
    secondary.websocket = { isActive: () => false };
    const status: Record<string, unknown> = {};
    let readback: Record<string, unknown> = status;
    const setStatus = vi.fn((patch: Record<string, unknown>) => Object.assign(status, patch));
    const getStatus = () => readback;

    const disposePrimary = installSlackSocketModeStatusObserver(
      { client: primary },
      setStatus,
      getStatus,
      { connectionId: "primary", activeProbeIntervalMs: 0 },
    );
    const disposeSecondary = installSlackSocketModeStatusObserver(
      { client: secondary },
      setStatus,
      getStatus,
      { connectionId: "socket-2", activeProbeIntervalMs: 0 },
    );
    primary.emit("connected");
    secondary.emit("connected");
    const staleTwoSocketStatus = structuredClone(status);

    disposeSecondary();
    expect(status).toMatchObject({ socketConnectionCount: 1 });
    expect(status.socketConnections).not.toHaveProperty("socket-2");

    readback = staleTwoSocketStatus;
    primary.emit("connected");

    expect(status).toMatchObject({
      connected: true,
      socketConnectionCount: 1,
      socketConnections: { primary: { connected: true } },
    });
    expect(status.socketConnections).not.toHaveProperty("socket-2");
    disposePrimary();
  });
