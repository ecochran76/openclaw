...
describe("embedded agent tool approval payload helpers", () => {
  ...
  it("reads a2a approval payloads and reuses the structured reply builder", () => {
    ...
    const approval = readA2APermissionApprovalDetails(result, "session_status");

    expect(approval).not.toBeNull();
    expect(readA2APermissionApprovalDetails(result, "plugin_status")).toBeNull();
    expect(readA2APermissionApprovalDetails(result, "sessions_send")).toBeNull();
    expect(
      buildExecApprovalReplyPayload({
        details: {
          approvalId: "approval-1",
