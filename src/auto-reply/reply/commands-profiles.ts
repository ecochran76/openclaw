...
export const handleProfilesCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  const nonOwner = rejectNonOwnerCommand(params, "/profiles");
  if (nonOwner) {
    return nonOwner;
  }
...
export const handleProfileCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  const unauthorized = rejectUnauthorizedCommand(params, "/profile");
  if (unauthorized) {
    return unauthorized;
  }
  const nonOwner = rejectNonOwnerCommand(params, "/profile");
  if (nonOwner) {
    return nonOwner;
  }
  ...
  if (parsed.kind === "clear") {
    if (!params.sessionEntry || !params.sessionStore || !params.sessionKey) {
      return {
        shouldContinue: false,
        reply: { text: "⚠️ Cannot clear the profile override before this session is persisted." },
      };
    }
    const current = params.sessionEntry.authProfileOverride?.trim();
    if (parsed.provider && current) {
      const requestedProvider = normalizeAuthCommandProvider(parsed.provider);
      const activeProvider = store.profiles[current]?.provider;
      if (!activeProvider || normalizeAuthCommandProvider(activeProvider) !== requestedProvider) {
        return {
          shouldContinue: false,
          reply: {
            text: `⚠️ Session profile override "${current}" is not for ${requestedProvider}; it was not cleared.`,
          },
        };
      }
    }
  ...
  const provider = resolveTargetProvider(parsed.provider, params.provider);
  const profileId = normalizeRequestedProfileId(provider, parsed.profileId) ?? parsed.profileId;
  const entry = store.profiles[profileId];
  if (!entry) {
    return {
      shouldContinue: false,
      reply: { text: `⚠️ Auth profile "${profileId}" not found.` },
    };
  }
  if (normalizeAuthCommandProvider(entry.provider) !== provider) {
    return {
      shouldContinue: false,
      reply: {
        text: `⚠️ Auth profile "${profileId}" is for ${entry.provider}, not ${provider}.`,
      },
    };
  }
  ...
  if (!params.sessionEntry || !params.sessionStore || !params.sessionKey) {
    return {
      shouldContinue: false,
      reply: { text: "⚠️ Cannot set a profile override before this session is persisted." },
    };
  }
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `👤 Session profile override set to ${profileId} (${provider}).`,
    },
  };
};
