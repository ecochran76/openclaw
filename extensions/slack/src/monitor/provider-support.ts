...
type SlackStatusWriter = (next: Record<string, unknown>) => void;
type SlackSocketConnectionWriterState = {
  connections: Record<string, Record<string, unknown>>;
  retiredConnectionIds: Set<string>;
};
const socketConnectionStateByStatusWriter = new WeakMap<
  SlackStatusWriter,
  SlackSocketConnectionWriterState
>();

function getSocketConnectionWriterState(setStatus: SlackStatusWriter) {
  const existing = socketConnectionStateByStatusWriter.get(setStatus);
  if (existing) {
    return existing;
  }
  const created: SlackSocketConnectionWriterState = {
    connections: {},
    retiredConnectionIds: new Set(),
  };
  socketConnectionStateByStatusWriter.set(setStatus, created);
  return created;
}

function mergeSocketConnectionStatus(params: {
  setStatus: SlackStatusWriter;
  snapshot?: Record<string, unknown>;
  connectionId: string;
  patch: Record<string, unknown>;
}): Record<string, Record<string, unknown>> {
  const current = params.snapshot?.socketConnections;
  const writerState = getSocketConnectionWriterState(params.setStatus);
  writerState.retiredConnectionIds.delete(params.connectionId);
  const connections = {
    ...writerState.connections,
    ...(current && typeof current === "object" && !Array.isArray(current)
      ? (current as Record<string, Record<string, unknown>>)
      : {}),
  };
  for (const retiredConnectionId of writerState.retiredConnectionIds) {
    delete connections[retiredConnectionId];
  }
  ...
  writerState.connections = connections;
  return connections;
}
...
function publishSocketConnectionPatch(params: {
  ...
  const activeState =
    params.patch.connected === false
      ? { state: "inactive" as const, available: true }
      : resolveSlackSocketActiveState(params.receiver);
...
function publishSlackSocketErrorStatus(params: {
  receiver: unknown;
  setStatus: (next: Record<string, unknown>) => void;
  getStatus?: () => Record<string, unknown>;
  connectionId: string;
  ...
  const at = Date.now();
  const message = formatUnknownError(params.error);
  publishSocketConnectionPatch({
    receiver: params.receiver,
    setStatus: params.setStatus,
    getStatus: params.getStatus,
...
export function installSlackSocketModeStatusObserver(
  ...
  opts?: {
    ...
    onExpectedRefresh?: () => void;
  },
): () => void {
  if (!setStatus && !opts?.onExpectedRefresh) {
    return () => {};
  }
  ...
  if (setStatus) {
    ...
    on("error", (error: unknown) =>
      publishSlackSocketErrorStatus({ receiver, setStatus, getStatus, connectionId, error }),
    );
  }
  ...
  on("ws_message", (data: unknown) => {
    ...
    if (disconnectReason) {
      ...
      if (classified.expectedRefresh) {
        opts?.onExpectedRefresh?.();
      }
      disconnectStatus = {
        at,
  ...
  return () => {
    ...
    if (setStatus) {
      const writerState = getSocketConnectionWriterState(setStatus);
      writerState.retiredConnectionIds.add(connectionId);
      delete writerState.connections[connectionId];
      setStatus({
        ...summarizeSocketConnections(writerState.connections),
        socketConnections: { ...writerState.connections },
      });
    }
  };
}
