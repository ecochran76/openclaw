...
```text
[slack] [default] channel exited: Cannot find module '/home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/dist/extensions/node_modules/openclaw/plugin-sdk/text-runtime.js' imported from /home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/dist/extensions/slack/send-BjtzUy1x.js
...
[plugins] slack failed during register from .../dist/extensions/slack/index.js: Error: Cannot find module '/home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/node_modules/typebox/build/index.mjs'
[channels] failed to load bundled channel slack: Cannot find module '/home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/node_modules/typebox/build/index.mjs'
...
OpenClaw 2026.4.27 reproduced the same event-loop/socket failure mode in a two-Slack-tenant setup. The example tenant `#website` channel was correctly configured for the `example-website` agent and the user message was visible in Slack history, but the inbound event never entered the agent session while the gateway was cycling Slack socket startup.

Observed sequence:

1. An allowlisted user directly mentioned the bot in the bound `#website`
   channel; the private message body is intentionally omitted.
2. The message came from an allowlisted user, directly mentioned the example tenant bot, and matched the configured `example-website` binding.
3. The prior `example-website` session only contained the earlier `17:25` CDT abilities prompt; the Canva prompt was absent from the session JSONL.
...
6. The replayed turn eventually completed and delivered to example tenant `#website`, proving the binding and policy were valid.
