...
- `openclaw channels status --json` after the post-install probe showed example tenant
...
- Controlled post-install Slack probe:
  `https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000`
  advanced example tenant receiver telemetry and was classified by installed
...
- Earlier controlled thread proof for
  `thread_ts=<slack-ts-redacted>` returned `admitted=1`, `not-relevant=1`,
  `missing-admission=0`.
...
- Do not add example tenant-specific health policy to core.
