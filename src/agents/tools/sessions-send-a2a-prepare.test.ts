...
describe("flow-prepare", () => {
  ...
  it("does not resolve announce targets while relay delivery is disabled", async () => {
    ...
    expect(prepared.flowParams.sourceRelayTarget).toBeNull();
    expect(prepared.flowParams.targetRelayTarget).toBeNull();
    expect(prepared.defaultRelay.status).toBe("disabled");
  });
