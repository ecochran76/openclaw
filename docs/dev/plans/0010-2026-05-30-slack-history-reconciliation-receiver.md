...
Source implementation for Phases 1-6 is present and live example tenant proof completed
as of 2026-05-31.

The example tenant miss at `C_REDACTED_PRIMARY` / `<slack-ts-redacted>` proves the gap:

- Slack history contains `[message text redacted]`.
...
- Do not add example tenant-specific channel policy.
...
- The known example tenant message classifies as `missing-admission` before recovery and
...
- Enable reconciliation for example tenant `ask-lei` in dry-run mode first.
...
- Then enable `autoRecover` for the configured example tenant operator-facing channel.
...
- Live example tenant proof shows no duplicate replies and successful recovery of a
...
2026-05-31 live example tenant proof:
...
- Enabled example tenant reconciliation in dry-run first with `autoRecover: false`,
  ...
  candidates after scanning the configured example tenant channel set.
- Rechecked the original unanswered permalink with
  `openclaw channels watchdog-scan --account example-account --permalink https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000`.
  It remains a historical miss: `missing-admission=1` for
  `<slack-ts-redacted>`, reason `activation-without-ledger-record`. That message
  is outside the current reconciliation lookback, so it is incident evidence,
  not a current health failure.
- Simulated a missed event by stopping the gateway, posting an explicit example tenant
  ...
  `bot_id` while example tenant production policy had `allowBots: false`.
- Temporarily set example tenant `allowBots: "mentions"` and `lookbackMs: 30000` for a
  controlled recovery proof, stopped the gateway, posted
  `<slack-ts-redacted>` while offline, and restarted the gateway. Reconciliation
...
- Operator proof for the recovered probe:
  ...
  in 27 seconds.
- Restored example tenant production policy after the controlled proof:
...
- Added final live coverage for the named probe classes in the `website`
  channel (`C_REDACTED_SECONDARY`), which has `requireMention: true`:
  - top-level parent `<slack-ts-redacted>` had no example agent mention and was recorded as
    `admission-ledger:dropped`; watchdog reported `explicitly-ignored=1` and
    `missing-admission=0`;
  - threaded explicit mention `<slack-ts-redacted>` was recorded as
    `admission-ledger:accepted`, entered the normal Slack turn path, and
    received a thread reply at `<slack-ts-redacted>`;
  - reconciliation later observed the bot reply as `bot-self`.
    Final example tenant status remained healthy with two Socket Mode connections,
