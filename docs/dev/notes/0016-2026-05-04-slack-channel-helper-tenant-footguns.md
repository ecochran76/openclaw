...
During example tenant company-bot channel setup, the local
...
Two accidental channels were created in the default Slack workspace before the
correct example-tenant token was used. Both accidental channels were
archived and their local note-store mappings were removed.

The intended example tenant channel was created only after overriding the helper with
the example tenant Slack user token. The example tenant bot token authenticated successfully
...
The example tenant channel create path was retried with the example tenant user token and
individual invites for three authorized test users.
