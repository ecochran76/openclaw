...
type AdmissionReport = {
  verdict:
    | "admitted"
    | "no-openclaw-admission-record"
    | "linked-message-not-found"
    | "inconclusive-partial-scan"
    | "not-scanned";
  explanation: string;
  evidence: string[];
};
...
export function parseSlackPermalink(raw: string): ParsedSlackPermalink {
  ...
  const rawMessageTs = match[2];
  if (!channelId || !rawMessageTs) {
    throw new Error("Slack permalink is missing its channel or timestamp component.");
  }
...
function textPreview(raw: unknown): string | undefined {
  ...
  const collapsed = redactSensitiveText(raw).replace(/\s+/g, " ").trim();
  if (!collapsed) {
    return undefined;
  }
...
function messageSummary(
  ...
  return {
    ...
    ...(includeTextPreview && textPreview(message.text)
...
export function buildRelatedSlackMessages(params: {
  ...
  const add = (role: RelatedSlackMessage["role"], message: MessageLike | undefined) => {
    ...
    related.push(messageSummary(role, message, params.includeTextPreviews === true));
  };
  add(
    "linked",
  ...
  const linkedMessage = params.linkedMessage
    ? {
        ...
        ...(typeof params.linkedMessage.subtype === "string"
          ...
          : {}),
        ...(params.includeTextPreview === true && textPreview(params.linkedMessage.text)
...
export function buildAdmissionReport(params: {
  ...
  const stableNeedles = new Set(
    [params.linkedMessage.ts, params.linkedMessage.client_msg_id].filter((value): value is string =>
      Boolean(value),
    ),
  );
  const evidence = params.fileMatches.flatMap((match) =>
    match.hits
      .filter((hit) => stableNeedles.has(hit))
      .map((hit) => `${path.basename(match.path)}:${hit}`),
  );
  if (evidence.length > 0) {
    return {
      verdict: "admitted",
      explanation: "A transcript or trajectory sidecar contains a stable linked-message id.",
      evidence,
    };
  }
  if (params.scanComplete === false) {
    return {
      ...
      explanation:
        "No stable linked-message id was found in the bounded transcript scan, but additional matching sessions were not scanned.",
      evidence: [],
    };
  }
  return {
...
export function redactInspectLinkFileMatches(params: {
  fileMatches: FileMatch[];
  linkedTextNeedles: string[];
}): FileMatch[] {
  const linkedTextNeedles = new Set(params.linkedTextNeedles);
  return params.fileMatches.map((match) =>
    Object.assign({}, match, {
      hits: match.hits.map((hit) => (linkedTextNeedles.has(hit) ? "[redacted-text-match]" : hit)),
    }),
  );
}
...
export async function channelsInspectLinkCommand(
  ...
  const ingress = buildInspectLinkIngressReport({
    ...
    account,
    linkedMessage,
    includeTextPreview: opts.messagePreviews === true,
  });
  const relatedSlackMessages = buildRelatedSlackMessages({
    parsed,
    messages,
    includeTextPreviews: opts.messagePreviews === true,
  });
  ...
  const scannedSessionMatches = sessionMatches.slice(0, 20);
  const scanComplete = scannedSessionMatches.length === sessionMatches.length;
  const fileMatches = await collectFileMatches({ matches: scannedSessionMatches, needles });
  const admission = buildAdmissionReport({
    linkedMessage,
    linkedNeedles,
    fileMatches,
    scanComplete,
  });
  const reportFileMatches = redactInspectLinkFileMatches({
    fileMatches,
    linkedTextNeedles: textNeedles(linkedMessage?.text),
  });
