...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/openclaw-agent"}/openclaw-agent.sqlite`,
    ),
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/openclaw-agent/openclaw-agent.sqlite");
  ...
  it("honors OPENCLAW_AGENT_DIR when no --agent override is provided", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/openclaw-isolated-agent/openclaw-agent.sqlite");
  });
    ...
    await withAgentScopeOverrides(
      ...
      async () => {
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/openclaw-agent-custom/openclaw-agent.sqlite",
        });
      },
    );
  });
  ...
  it("reports Gemini CLI OAuth for canonical Google text routed through the CLI runtime", async () => {
    ...
    try {
      ...
      expect(
        ...
        kind: "profiles",
        detail: "/tmp/openclaw-agent/openclaw-agent.sqlite",
      });
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          provider: "google",
