...
Native slash commands (Discord, Telegram, and other surfaces with native command support) and validated text control commands such as `/status` bypass `visibleReplies: "message_tool"` and always reply visibly so command UI and command-like text turns get the response they expect. Ordinary chat turns still follow the configured group default.
