...
Lei (`example-primary`) so Lei can classify memories and write source-cited
Graphiti records. The live drain successfully wrote Graphiti episodes in group
`example_company`, but Lei did not receive a concrete OpenClaw memory write tool.

## Evidence

- Live `example-primary` config included `tools.profile: messaging` with
...
`company-bot` and Lei now treat Graphiti group `example_company` as the required
