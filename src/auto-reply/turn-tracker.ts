...
const RECENT_SESSION_LIMIT = 500;
export const TRACKED_TURN_STALL_THRESHOLD_MS = 120_000;

type TurnTrackerState = {
  activeBySession: Map<string, TrackedTurnSnapshot>;
  recentListBySession: Map<string, TrackedTurnSnapshot[]>;
  turnIdToSession: Map<string, string>;
  runIdToTurnId: Map<string, string>;
};

const turnTrackerStateKey = Symbol.for("openclaw.auto-reply.turn-tracker.state");
// These maps form one lifecycle index: split runtime chunks must share all four
// or completion can archive a turn without clearing its reverse identities.
const { activeBySession, recentListBySession, turnIdToSession, runIdToTurnId } =
  resolveGlobalSingleton<TurnTrackerState>(turnTrackerStateKey, () => ({
    activeBySession: new Map(),
    recentListBySession: new Map(),
    turnIdToSession: new Map(),
    runIdToTurnId: new Map(),
  }));
...
function finalizeTrackedTurnSnapshot(params: {
  ...
  current.completedAt = params.completedAt;
  current.updatedAt = params.completedAt;
  current.lastProgressAt = params.completedAt;
  if (params.deliveryState !== undefined) {
    current.deliveryState = params.deliveryState;
  }
  const finalized = projectTrackedTurn(current, params.completedAt);
  if (activeBySession.get(current.sessionKey)?.turnId === current.turnId) {
    activeBySession.delete(current.sessionKey);
  }
  const recent = recentListBySession.get(current.sessionKey) ?? [];
  recent.unshift(finalized);
  // Refresh completion order so the bounded map evicts the least recently completed session.
  recentListBySession.delete(current.sessionKey);
  recentListBySession.set(current.sessionKey, recent.slice(0, RECENT_TURN_LIMIT));
  while (recentListBySession.size > RECENT_SESSION_LIMIT) {
    const oldestSessionKey = recentListBySession.keys().next().value;
    if (oldestSessionKey === undefined) {
      break;
    }
    recentListBySession.delete(oldestSessionKey);
  }
  if (current.runId) {
    runIdToTurnId.delete(current.runId);
  }
  turnIdToSession.delete(current.turnId);
  return finalized;
}
...
export function startTrackedTurn(params: {
  ...
  const previous = activeBySession.get(params.sessionKey);
  if (previous) {
    // A session exposes one active turn. Archive the displaced turn and clear
    // both reverse indexes so its eventual late completion cannot remove the new turn.
    finalizeTrackedTurnSnapshot({
      snapshot: previous,
      status: "error",
      phase: "error",
      completedAt: Math.max(startedAt, previous.startedAt, previous.updatedAt),
...
export function updateTrackedTurn(
  ...
  if ("activeTool" in patch) {
    if (patch.activeTool) {
      current.activeTool = patch.activeTool;
    } else {
      delete current.activeTool;
    }
  }
  ...
  if ("lastDeliveryError" in patch) {
    if (patch.lastDeliveryError) {
      current.lastDeliveryError = patch.lastDeliveryError;
    } else {
      delete current.lastDeliveryError;
    }
  }
...
export function finishTrackedTurn(params: {
  ...
  const status = params.status ?? (params.error ? "error" : "done");
  const finalized = finalizeTrackedTurnSnapshot({
    snapshot: current,
    status,
    phase: params.phase ?? (status === "error" ? "error" : "done"),
    error: params.error,
