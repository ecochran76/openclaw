...
OpenClaw code change is required for the observed example tenant heartbeat incident.
...
Corrected conclusion: OpenClaw dispatched `odollo-example` and read the correct
...
incorrect for the observed example tenant case. The scheduler was dispatching
`odollo-example` on the 30-minute cadence, using session key
`agent:odollo-example:main`, and reading the correct workspace heartbeat file:

```text
/home/user/.odollo/openclaw/agents/example-prod-odollo-agent/HEARTBEAT.md
```
...
```text
trigger: heartbeat
sessionKey: agent:odollo-example:main
workspaceDir: /home/user/.odollo/openclaw/agents/example-prod-odollo-agent
toolMetas: read from ~/.odollo/openclaw/agents/example-prod-odollo-agent/HEARTBEAT.md
...
```text
/home/user/.odollo/tenants/example-prod/artifacts/monitor-tick-heartbeat-2026-04-24.json
/home/user/.odollo/tenants/example-prod/artifacts/crm-monitor-tick-reviewed-2026-04-24-heartbeat-smoke.json
/home/user/.odollo/tenants/example-prod/artifacts/crm-monitor-events-2026-04-24.jsonl
```
...
- Affected agent id: `odollo-example`
- Agent workspace:
  `/home/user/.odollo/openclaw/agents/example-prod-odollo-agent`
...
```json
{
  "id": "odollo-example",
  "name": "odollo-example",
  "workspace": "/home/user/.odollo/openclaw/agents/example-prod-odollo-agent",
  "agentDir": "/home/user/.openclaw/agents/odollo-example/agent",
  "model": "openai-codex/gpt-5.4-mini",
  "identity": {
    "name": "Odollo example tenant"
  },
  "heartbeat": {
    "every": "30m"
  }
...
```text
Heartbeat: 30m (main), ..., 30m (odollo-example), ...
...
root-cause statement for the example tenant incident.

### Original Observed Behavior

After a manual recovery turn completed at `2026-04-24T12:59:20Z`, no automatic
`odollo-example` heartbeat turn ran by `2026-04-24T15:10:33-05:00`, despite the
...
The task was `agent:odollo-example:heartbeat-smoke`, created on
...
openclaw agent \
  ...
  --session-key agent:odollo-example:heartbeat-recovery-20260424 \
  --message 'Heartbeat recovery check after clearing stale task...'
```

The manual run succeeded:

- run id: `RUN_ID_REDACTED`
- task id: `RUN_ID_REDACTED`
...
- monitor packet:
  `/home/user/.odollo/tenants/example-prod/artifacts/monitor-tick-heartbeat-2026-04-24.json`
- reviewed output:
  `/home/user/.odollo/tenants/example-prod/artifacts/crm-monitor-tick-reviewed-2026-04-24-0757cdt.json`
- accumulator:
  `/home/user/.odollo/tenants/example-prod/artifacts/crm-monitor-events-2026-04-24.jsonl`
...
The gateway recognizes `30m (odollo-example)` in status output, and manual
`openclaw agent --agent odollo-example ...` works, but automatic interval
...
the non-empty `odollo-example` workspace `HEARTBEAT.md`.
...
- The example tenant incident is not treated as an OpenClaw dispatch failure.
