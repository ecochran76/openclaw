...
import {
  buildChannelsWhySilentReport,
  channelsWhySilentCommand,
  formatChannelsWhySilentReport,
} from "./why-silent.js";
...
describe("buildChannelsWhySilentReport", () => {
  ...
  it("does not report a socket error superseded by newer receiver activity", () => {
    const report = buildChannelsWhySilentReport({
      channel: "slack",
      accountId: "soylei",
      target: "channel:C123",
      ...
      account: {
        ...
        lastSocketConnectedAt: 1_700_000_000_000,
        lastSocketError: { at: 1_700_000_009_000, error: "socket failed" },
        lastSocketEnvelopeAt: 1_700_000_009_500,
      },
      messages: [
        {
          ts: "1700000006.500000",
  ...
  it("does not use Slack-specific receiver wording for other channel outages", () => {
    ...
    expect(report.explanation).toContain("The channel history");
    expect(report.explanation).not.toContain("Slack history");
...
describe("channelsWhySilentCommand", () => {
  it("unwraps a gateway channels.status envelope before resolving account diagnostics", async () => {
    const logs: unknown[] = [];
    const callGateway = vi
      ...
      .mockResolvedValueOnce({
        payload: {
          messages: [{ ts: "1700000006.500000", user: "U123", text: "hello" }],
        },
      });

    await channelsWhySilentCommand(
      { channel: "discord", target: "channel:C123", json: true },
      { log: (value: string) => logs.push(JSON.parse(value) as unknown) } as never,
      { callGateway },
    );
    ...
    expect(logs).toContainEqual(
      expect.objectContaining({
        ...
        account: expect.objectContaining({ running: true, connected: true }),
      }),
    );
  });
});
