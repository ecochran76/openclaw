...
import { AgentTotalTokenBudgetExceededError } from "../embedded-agent-runner/run/attempt-total-token-budget.js";
...
function createDefaultWorkerTurnExecutor(opts: AutomationToolOptions, cliDeps?: AutomationCliDeps) {
  ...
  return async (input: AutomationWorkerTurnInput): Promise<AutomationWorkerTurnResult> => {
    ...
    const execution = buildAutomationWorkerExecution(input);
    let runResult: AutomationWorkerRunResult;
    try {
      runResult = await runCronIsolatedAgentTurn({
        cfg,
        ...
        streamParams: execution.streamParams,
        abortSignal: input.abortSignal,
      });
    } catch (error) {
      if (error instanceof AgentTotalTokenBudgetExceededError) {
  ...
  return {
    ...
    execute: async (_toolCallId, args) => {
      ...
      if (action === "run") {
        ...
        const startResult = startAutomationRunInBackground({
          requesterSessionKey,
        ...
        if (startResult.status === "capacity") {
          return jsonResult({
            status: "error",
            error: `automation concurrency limit reached (${startResult.activeCount}/${startResult.maxConcurrent})`,
            activeRunIds: startResult.activeRunIds,
          });
        }
        const record = startResult.run;
        return jsonResult({
          status: "accepted",
          runId: record.runId,
