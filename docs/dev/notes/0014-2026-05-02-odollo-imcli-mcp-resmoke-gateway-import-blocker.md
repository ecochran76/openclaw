...
gateway-dispatched `odollo-example` agent smoke failed before agent execution
...
- The same configured command and cwd as OpenClaw were used:
  `/home/user/.local/share/pnpm/imcli --tenant default mcp`
  with cwd `/home/user/workspace.local/imcli`.
...
odollo-example ...` smoke failed through the gateway path:

```text
EMBEDDED FALLBACK: Gateway agent failed; running embedded agent: GatewayClientRequestError: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/dist/transcript-resolve.runtime-Djgr04ZR.js' imported from /home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/dist/agent-command-CvybDRXZ.js
```

Gateway logs also showed a related lazy import failure:

```text
[model-catalog] Failed to load model catalog: Error [ERR_MODULE_NOT_FOUND]: Cannot find module '/home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/dist/provider-runtime-B3qmO6yf.js' imported from /home/user/.nvm/versions/node/v24.13.0/lib/node_modules/openclaw/dist/provider-runtime.runtime-BRJDRgFx.js
```
...
A live `odollo-example` agent smoke completed without the old missing-module
...
- `agents.list[].id=odollo-example` has `tools.profile: coding` and
  `tools.alsoAllow` includes `imcli__*`.
- `mcp.servers.imcli` points at `/home/user/.local/share/pnpm/imcli
--tenant default mcp` with cwd `/home/user/workspace.local/imcli`.
...
- Treat live OpenClaw MCP tool exposure for `odollo-example` as fixed by local
...
gateway service `PATH` also caused the `/home/user/.local/share/pnpm/imcli`
...
`/home/user/.nvm/versions/node/v24.13.0/bin` before `/usr/bin`.
...
openclaw agent --agent odollo-example --session-key agent:odollo-example:imcli-smoke-fixed ...
```
...
A non-delivered read-only `odollo-example` agent smoke completed through the
...
The runtime note at
`/home/user/.openclaw/docs/dev/notes/0002-2026-05-02-odollo-imcli-tool-exposure-handoff.md`
...
- Prefer a live gateway smoke that runs `odollo-example` with `imcli__list_accounts`
...
```text
/home/user/workspace.local/odollo/doc/dev/notes/openclaw-latency-testing-handoff-2026-05-02.md
```
