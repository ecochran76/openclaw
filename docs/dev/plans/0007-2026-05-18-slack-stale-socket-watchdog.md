...
On 2026-05-20, example tenant `#ask-lei` showed the same missing-admission shape for
`/status` at `<slack-ts-redacted>`: Slack history contained the message, gateway
status reported the example tenant Socket Mode account as connected/healthy, but
...
2. Read-only detector.
   ...
   - 2026-05-20: the scanner and `channels watchdog-scan` wrapper were updated
     ...
     the configured channel user allowlist. The example tenant `#ask-lei` Baker incident
...
```text
OpenClaw missed 1 Slack message that looked eligible for example agent.

Where: example tenant #ask-lei
...
```text
OpenClaw missed this message before it reached example agent. I have flagged it for recovery so it is not silently ignored.
```
...
```text
Recovery complete: OpenClaw has replayed this missed message into example agent.
```
...
- CLI dry-run proof:
  - `openclaw channels watchdog-scan --account example-account --target channel:C_REDACTED_PRIMARY --since 30m --json`
...
pnpm openclaw channels watchdog-scan --account example-account --target channel:C_REDACTED_PRIMARY --since 24h --limit 100 --bot-user U_REDACTED_BOT --dry-run-replies --json --timeout 15000
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts
node scripts/run-vitest.mjs extensions/slack/src/monitor/watchdog-scan.test.ts extensions/slack/src/monitor/admission-ledger.test.ts
...
scripts/ec-main-rebase-gate.sh --family slack-responsiveness
pnpm openclaw channels watchdog-scan --account example-account --target channel:C_REDACTED_PRIMARY --since 24h --limit 100 --bot-user U_REDACTED_BOT --dry-run-replies --json --timeout 15000
pnpm openclaw channels watchdog-replay --account example-account --target channel:C_REDACTED_PRIMARY --ts <slack-ts-redacted> --since 24h --limit 100 --bot-user U_REDACTED_BOT --json --timeout 15000
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts extensions/slack/src/monitor/watchdog-scan.test.ts extensions/slack/src/monitor/admission-ledger.test.ts
scripts/ec-main-rebase-gate.sh --family slack-responsiveness
scripts/ec-main-rebase-gate.sh --family slack-responsiveness
...
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts extensions/slack/src/monitor/watchdog-scan.test.ts extensions/slack/src/monitor/admission-ledger.test.ts
scripts/ec-main-rebase-gate.sh --family slack-responsiveness
pnpm openclaw channels watchdog-replay --account example-account --permalink https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000 --since 24h --limit 100 --bot-user U_REDACTED_BOT --json --timeout 15000
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts extensions/slack/src/monitor/watchdog-scan.test.ts extensions/slack/src/monitor/admission-ledger.test.ts
scripts/ec-main-rebase-gate.sh --family slack-responsiveness
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts extensions/slack/src/monitor/watchdog-scan.test.ts extensions/slack/src/monitor/admission-ledger.test.ts
...
openclaw channels watchdog-status --account example-account --json
openclaw channels watchdog-replay --account example-account --permalink https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000 --since 24h --limit 100 --bot-user U_REDACTED_BOT --json --timeout 15000
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts extensions/slack/src/monitor/watchdog-scan.test.ts extensions/slack/src/monitor/admission-ledger.test.ts
...
openclaw channels watchdog-scan --account example-account --permalink https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000 --since 24h --limit 100 --bot-user U_REDACTED_BOT --dry-run-replies --max-replies 10 --json --timeout 15000
openclaw channels watchdog-scan --account example-account --permalink https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000 --since 24h --limit 100 --bot-user U_REDACTED_BOT --reply-missed --max-replies 1 --json --timeout 15000
openclaw channels watchdog-status --account example-account --json
openclaw channels watchdog-scan --account example-account --permalink https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000 --since 24h --limit 100 --bot-user U_REDACTED_BOT --dry-run-replies --max-replies 10 --json --timeout 15000
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts
node scripts/run-vitest.mjs src/commands/channels/slack-watchdog-scan.test.ts extensions/slack/src/monitor/watchdog-scan.test.ts extensions/slack/src/monitor/admission-ledger.test.ts
...
Latest live direct Slack-plugin scan dry-run result: scanned 10 recent example tenant
...
example tenant `#ask-lei` message `<slack-ts-redacted>` classified as
`missing-admission`, produced `outcome=dry-run`, resolved `agentId` to
`example-account-primary`, and resolved the target delivery thread to
`channel:C_REDACTED_PRIMARY` / `<slack-ts-redacted>` without starting an agent turn.
...
`https://example.slack.com/archives/C_REDACTED_PRIMARY/p0000000000000000` returned
`outcome=dry-run`, `verdict=missing-admission`, `agentId=example-account-primary`, and
delivery `channel:C_REDACTED_PRIMARY` / thread `<slack-ts-redacted>` without dispatching
an agent turn. Installed `watchdog-status --account example-account --json` also
...
`--reply-missed --max-replies 1` against the same example tenant permalink sent exactly
one source-thread notice for message `<slack-ts-redacted>` in
`channel:C_REDACTED_PRIMARY`, with `sourceReplies.sent=1`, `failed=0`, and the
thread target `<slack-ts-redacted>`. `watchdog-status --account example-account --json`
...
returns `channel_not_found` for `channel:C_REDACTED_UNAVAILABLE` under account `example-account`.
