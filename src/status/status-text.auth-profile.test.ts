...
describe("resolveStatusModelAuthLabel", () => {
  ...
  it("surfaces agent-local auth order drift from main", async () => {
    ...
    ).toBe("oauth (openai:pcg) · agent order prefers openai:pcg; main last-good openai:soylei");
  });
  ...
  it("preserves the runtime label when no stored profile was selected", async () => {
    resolveModelAuthLabel.mockReturnValue("api-key (OPENAI_API_KEY)");
    ...
    ).toBe("api-key (OPENAI_API_KEY)");
  });

  it("surfaces active auth failure state for the selected profile", async () => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-05-15T19:00:00Z"));
    resolveModelAuthLabel.mockReturnValue("oauth (openai-codex:pcg)");
    resolveAuthProfileOrder.mockReturnValue(["openai-codex:pcg"]);
    ensureAuthProfileStore.mockReturnValue({
      profiles: {
        "openai-codex:pcg": {
