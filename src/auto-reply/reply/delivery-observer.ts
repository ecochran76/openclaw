...
export type DeliveryObserver = {
  ...
  hasVisibleDeliveryOutcome: () => boolean;
...
export function createDeliveryObserver(params: {
  ...
  const hasVisibleDeliveryOutcome = () => {
    if (!trackedTurnId || !params.sessionKey) {
      return false;
    }
    const snapshot = getActiveTrackedTurn(params.sessionKey);
    if (
      snapshot?.turnId !== trackedTurnId ||
      !snapshot.replyProduced ||
      snapshot.lastDeliveryAttemptAt === undefined
    ) {
      return false;
    }
    return (
      snapshot.lastDeliverySuccessAt !== undefined ||
      snapshot.deliveryState === "delivery_failed" ||
      snapshot.lastDeliveryError !== undefined
    );
  };
  ...
  const finalizeTrackedTurnDeliveryState = (explicit?: FinishTrackedTurnDeliveryState) => {
    if (!trackedTurnId) {
      return { snapshot: undefined, derived: undefined };
    }
    const snapshot = params.sessionKey ? getActiveTrackedTurn(params.sessionKey) : undefined;
    if (!snapshot || snapshot.turnId !== trackedTurnId) {
      return { snapshot: undefined, derived: undefined };
    }
        ...
        () => {
          ...
          void (async () => {
            ...
            const sendWatcherPayload = async (payload: ReplyPayload, failureText: string) => {
              if (!(await params.onSendWatcherPayload(payload, failureText))) {
            ...
            const active =
              trackedTurnId && params.sessionKey
                ? getActiveTrackedTurn(params.sessionKey)
                : undefined;
            if (active?.turnId === trackedTurnId) {
              // A watcher transport failure must not become an unhandled rejection.
              // Keep the active turn observable by retrying at the normal cadence.
              schedule(90_000);
            } else {
              clearTurnNudgeTimer();
            }
  ...
  const finishRun: DeliveryObserver["finishRun"] = (opts) => {
    ...
    const active = params.sessionKey ? getActiveTrackedTurn(params.sessionKey) : undefined;
    if (active && active.turnId !== trackedTurnId) {
      // A newer run owns the session now. This observer must not finalize or derive delivery
      // state from the superseding turn, including a false stranded-reply notice.
