...
export const SLACK_RECONCILIATION_STATE_NAMESPACE = "reconciliation";
export const SLACK_RECONCILIATION_STATE_MAX_ENTRIES = 1_000;
...
export function openSlackReconciliationStateStore(
...
): PluginStateKeyedStore<SlackReconciliationState> {
  return openKeyedStore<SlackReconciliationState>({
    namespace: SLACK_RECONCILIATION_STATE_NAMESPACE,
    maxEntries: SLACK_RECONCILIATION_STATE_MAX_ENTRIES,
...
function normalizeChannelState(value: unknown): SlackReconciliationChannelState | undefined {
  ...
  if (typeof record.lastThreadScanAt === "string" && record.lastThreadScanAt.trim()) {
    out.lastThreadScanAt = record.lastThreadScanAt.trim();
  }
...
export function normalizeSlackReconciliationState(
  value: Partial<SlackReconciliationState>,
  accountId: string,
): SlackReconciliationState {
  if (value.version !== 1 || value.accountId !== accountId) {
    return createEmptySlackReconciliationState(accountId);
  }
  const state = createEmptySlackReconciliationState(accountId);
...
export async function readSlackReconciliationState(params: {
  ...
  const value: Partial<SlackReconciliationState> =
    (await openSlackReconciliationStateStore(params.openKeyedStore).lookup(params.accountId)) ?? {};
  return normalizeSlackReconciliationState(value, params.accountId);
}
...
export async function writeSlackReconciliationState(params: {
  ...
  // Reconciliation mutates optional fields back to undefined as cursors clear.
  // Normalize through JSON so the plugin-state value remains JSON-compatible.
  ...
  ) as SlackReconciliationState;
  await openSlackReconciliationStateStore(params.openKeyedStore).register(
    params.accountId,
    storedState,
  );
}
