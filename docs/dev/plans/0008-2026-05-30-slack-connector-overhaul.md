...
for operator-facing channels. Recent example tenant misses show Slack itself and
...
- 2026-05-30: Enabled source-thread replies in the live example tenant scheduled
  ...
  alert paths stay enabled. `watchdog-status --account example-account --json` advanced
  `sourceReplied` from 1 to 2 after a write-gated scan for
  `<slack-ts-redacted>`; the repeat dry-run reported `sourceReplies.sent=0` and
...
- 2026-05-30: Live example tenant proof in `ask-lei`:
  ...
  example tenant `lastSocketEnvelopeAt`, `lastSlackEventAt`, and `slackTelemetry`
...
- 2026-05-30: Final review caught that raw Slack frames/events were still being
  ...
  `20260530T171441Z` showed example tenant raw receiver fields and counters without
...
- Keep the fix inside the Slack plugin and generic channel contracts; do not add
  example tenant-specific policy to core.
...
- Enable the live scheduled example tenant watchdog to use `--reply-missed` with a
...
- `openclaw channels watchdog-status --account example-account --json` increments
...
- Avoid core special cases for Slack account ids or example tenant channels.
...
- Send controlled messages in example tenant `ask-lei`:
