...
function normalizeResolvedDeliveryContext(row: GatewaySessionRow):
  ...
  // Slack timestamps are fractional identifiers whose trailing zeros are significant.
  // Preserve the protocol's canonical string instead of routing through numeric normalization.
  const rawThreadId = row.deliveryContext?.threadId ?? normalized.threadId;
  return {
...
export async function resolveSessionKeyFromResolveParams(params: {
  ...
  const { cfg, p } = params;
  if (p.threadId !== undefined && typeof p.threadId !== "string") {
    return {
      ok: false,
      error: errorShape(ErrorCodes.INVALID_REQUEST, "threadId must be a string"),
    };
  }
  ...
  const hasSelectorFilters =
    Boolean(
      channel || to || accountId || threadId || search || searchFields.length || threadPolicy,
  ...
  if (effectiveThreadPolicy === "most-recent" && selection === "least-recent") {
    return {
      ok: false,
      error: errorShape(
        ErrorCodes.INVALID_REQUEST,
        "threadPolicy=most-recent cannot be combined with selection=least-recent",
      ),
    };
  }
  ...
  if (hasSessionId) {
    ...
    const sessionIdSelection = resolveSessionIdMatchSelection(matches, sessionId);
    if (sessionIdSelection.kind === "none") {
      return noSessionFoundResult({ p, message: `No session found: ${sessionId}` });
    }
    if (sessionIdSelection.kind === "ambiguous") {
      const keys = sessionIdSelection.sessionKeys.join(", ");
      return {
        ok: false,
        error: errorShape(
          ErrorCodes.INVALID_REQUEST,
    ...
    const selectedEntry = matches.find(
      ([matchKey]) => matchKey === sessionIdSelection.sessionKey,
    )?.[1];
    const agentCheckSessionId = validateSessionAgentExists(
      cfg,
      sessionIdSelection.sessionKey,
      selectedEntry,
    );
    if (agentCheckSessionId) {
      return agentCheckSessionId;
    }
    return { ok: true, key: sessionIdSelection.sessionKey };
  }
  ...
  if (hasLabel) {
    ...
    const selected = expectDefined(list.sessions[0], "single label match");
  ...
  let matches = list.sessions.filter((row) => {
    ...
    const rowAccountId = resolveRowAccountId(row) ?? DEFAULT_ACCOUNT_ID;
  ...
  const selectedRow = expectDefined(selected[0], "resolved session selection");

  const agentCheckSelected = validateSessionAgentExists(cfg, selectedRow.key);
