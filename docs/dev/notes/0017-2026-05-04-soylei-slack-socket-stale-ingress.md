# example tenant Slack Socket Stale Ingress
...
The example tenant `#ask-lei` channel missed inbound messages while `openclaw channels status --probe` still reported Slack as connected and working. Slack history contained the user messages, but the Gateway logs had no corresponding ingress or delivery records for the example tenant channel.

The channel configuration was correct: the example tenant account was bound to `example-primary`, the channel policy allowed top-level messages without a mention, and authorized users were resolved. Restarting `openclaw-gateway.service` re-established Slack Socket Mode and inbound activity resumed.
