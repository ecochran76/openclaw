...
Successor: `docs/dev/plans/0023-2026-07-16-ec-main-recovery-integration-and-publication.md`

# ec-main Upstream Feature-Family Replay

## Cancellation

Plan 0016 is cancelled as an execution plan. Its original `/goal` expanded into
an unbounded combined replay/review loop: 28.7 hours, 10,546,555 tokens, and 42
autoreview cycles without an integration commit. Resuming this file would make
the drifted execution contract active again.

Completed work remains authoritative and is not discarded:

- the semantic replay candidate and containment refs;
- the drift audit in
  `docs/dev/notes/0035-2026-07-16-ec-main-rebase-goal-drift-containment.md`;
- closed blocker Plans 0017 through 0022 and their focused proof;
- exact post-blocker recovery ref
  `refs/backup/ec-main-drift-containment-20260716-six-blockers` at
  `12780b2009e5ce1f11e1ed40cd39220dbb483e6d`.

Only unfinished integration, validation, publication, live-patch, and model
transition work moves to Plan 0023. The remaining sections below are retained
as historical design and evidence, not as an active checklist.
...
## Recovery Audit | 2026-07-16

The original `/goal` is paused after 28.7 hours, 10,546,555 tokens, and 42
autoreview cycles. Do not resume that full-bundle loop. It reached a complete
semantic replay candidate but never produced integration commits, and the
temporary branch still points at the fixed upstream target.

Containment preserved three exact local refs:

- `refs/backup/ec-main-drift-containment-20260716-index` preserves the
  519-file index visible when the goal was contained;
- `refs/backup/ec-main-drift-containment-20260716-stash` preserves the Cycle 42
  path-scoped stash commit;
- `refs/backup/ec-main-drift-containment-20260716-full` preserves the restored
  573-file candidate before this audit note was added.

The Cycle 42 stash was restored path-by-path from its saved index. The result
has no unmerged paths, no unstaged or untracked files, and passes
`git diff --cached --check`. Preserve this candidate rather than rolling back
wholesale: the first autoreview found concrete correctness and security defects
that were subsequently repaired, while the late changes remain concentrated in
the planned feature families.

The containment audit identified six blockers before any commit or publication:

1. the release-tag auto-upgrade no-op exit can leave the checkout on the
   integration branch;
2. `sessions_list` leaks hidden agent-row existence through pre-authorization
   lookup;
3. cross-agent session selectors resolve targets before authorization;
4. a failure-notice dispatch can outlive an aborted source dispatch;
5. turn-tracker registries are module-local despite split runtime chunks;
6. reply-payload metadata uses a module-local weak map and can disappear across
   module copies for frozen payloads.

Resume with bounded family slices only: repair these six findings with focused
regressions, split the candidate along the six feature-family owner boundaries,
and run each slice's focused gate plus fresh autoreview until that bounded slice
has no actionable findings. Only then run the prescribed broad remote checks,
build, ancestry move, force-with-lease publication, live patch, and model
transition. Do not send the complete multi-megabyte replay through another
open-ended review loop.

Each blocker was delegated to its own serialized recovery plan and executed by
a dedicated subagent:

- Plan 0017: auto-upgrade no-op branch restoration;
- Plan 0018: `sessions_list` pre-authorization row oracle;
- Plan 0019: cross-agent selector authorization ordering;
- Plan 0020: abort-bound failure-notice delivery;
- Plan 0021: split-module turn-tracker singleton state;
- Plan 0022: split-module reply-payload metadata.

All six plans closed on 2026-07-16 with focused regression proof. Their
subagents edited and tested only their owned surfaces and did not stage, commit,
switch branches, rewrite refs, or broaden into unrelated replay work. The
family commit sequence may resume after the parent integrates the six slices
and records one exact recovery ref. Mandatory pre-commit autoreview remains a
family-slice gate; bounded isolated review attempts for Plans 0020 and 0022
returned no structured result before their recovery caps and were not retried.
