...
import {
  ...
  SLACK_ADMISSION_LEDGER_MAX_ENTRIES,
...
function needsBroadSlackChannelEnumeration(ctx: SlackMonitorContext): boolean {
  ...
  // Explicit channel entries refine those rooms; they do not narrow an open
  // group policy. Discovery must still include every joined room.
  return ctx.groupPolicy === "open" || keys.some((key) => key === "*");
}
...
async function listSlackScanChannels(
  ...
  do {
    if (shouldStop()) {
      throw new Error("slack history reconciliation stopped");
    }
...
async function resolveScanChannels(
  ...
  client: SlackConversationsClient,
  shouldStop: () => boolean,
): Promise<{ channels: string[]; discoveryErrors: unknown[] }> {
  const channels = new Set<string>();
  ...
  if (roomTypes.length > 0) {
    ...
    try {
      for (const channel of await listSlackScanChannels(
        client,
        roomTypes,
        shouldStop,
        allowedNames,
  ...
  if (conversationTypes.length > 0) {
    try {
      for (const channel of await listSlackScanChannels(client, conversationTypes, shouldStop)) {
        channels.add(channel);
      }
    } catch (error) {
      if (shouldStop()) {
  ...
  return { channels: Array.from(channels).sort(), discoveryErrors };
}
...
async function classifyRoomSenderAuthorization(params: {
  ...
  const botMessage = isSelfOrBotMessage(ctx, message);
  if (
    !botMessage &&
    allowFromLower.length > 0 &&
    !resolveSlackAllowListMatch({
      allowList: allowFromLower,
      id: senderId,
      name: senderName,
    ...
    return { allowed: false, reason: "room-sender-not-allowed" };
  }
  if (
    botMessage &&
  ...
  const fetchThreadReplies = async (root: {
    ...
    do {
      ...
      const page = await params.client.conversations.replies({
        ...
        // History pagination lowers `latest` while draining a backlog. Thread roots
        // already queued in that backlog retain its original high-water boundary.
        latest: params.threadLatest,
        inclusive: true,
...
export function startSlackHistoryReconciliation(params: {
  ...
  // Metadata hydration may finish after monitor shutdown. Honor an already-aborted
  // signal here because adding an abort listener later will not replay that event.
  let stopped = !config.enabled || params.abortSignal?.aborted === true;
  let timer: NodeJS.Timeout | undefined;
  let running = false;
  let activeRun: Promise<void> | undefined;
  const client = params.ctx.app.client as unknown as SlackConversationsClient;
  const shouldStop = () => stopped || params.abortSignal?.aborted === true;
  ...
  const performRunOnce = async () => {
    ...
    try {
      ...
      let channels: string[];
      try {
        const resolution = await resolveScanChannels(params.ctx, client, shouldStop);
        channels = resolution.channels;
        if (shouldStop()) {
          return;
        }
        if (resolution.discoveryErrors.length > 0) {
          const error = resolution.discoveryErrors[0];
          ...
          params.ctx.runtime.log?.(
            `slack history reconciliation discovery failed for ${params.accountId}: ${getSlackApiErrorCode(error)}; continuing ${channels.length} known channel scan(s)`,
          );
        }
      } catch (error) {
        if (shouldStop()) {
      ...
      let admissionRecords;
      try {
        admissionRecords = await readSlackAdmissionRecords({
          accountId: params.accountId,
          // Reconciliation must see the full retained ledger or older accepted messages can replay.
          limit: SLACK_ADMISSION_LEDGER_MAX_ENTRIES,
        });
        if (shouldStop()) {
          return;
        }
        ...
        params.ctx.runtime.log?.(
          `slack history reconciliation skipped for ${params.accountId}: admission state unavailable (${getSlackApiErrorCode(error)})`,
        );
        return;
      }
      ...
      // Oldest-first rotation prevents a busy low-sorting channel from owning every
      // capped cycle. The per-channel share reserves budget for later channels too.
      const fairScanChannels = channels.toSorted((left, right) => {
        const leftScannedAt = state.channels[left]?.lastScanAt;
        const rightScannedAt = state.channels[right]?.lastScanAt;
        if (leftScannedAt !== rightScannedAt) {
          if (!leftScannedAt) {
            return -1;
          }
          if (!rightScannedAt) {
          ...
          return leftScannedAt.localeCompare(rightScannedAt);
        }
        return left.localeCompare(right);
      });
      // Thread expansion has its own scarce account-wide budget. Rotate that budget
      // independently so history scans still run when a channel receives zero roots.
      const fairThreadChannels = channels.toSorted((left, right) => {
        const leftScannedAt = state.channels[left]?.lastThreadScanAt;
        const rightScannedAt = state.channels[right]?.lastThreadScanAt;
        if (leftScannedAt !== rightScannedAt) {
          if (!leftScannedAt) {
            return -1;
          }
      ...
      const threadRootBudgets = new Map<string, number>();
      for (const [channelIndex, channel] of fairThreadChannels.entries()) {
        const remainingChannels = fairThreadChannels.length - channelIndex;
        const budget =
          remainingCycleThreadRoots > 0
            ? Math.max(1, Math.floor(remainingCycleThreadRoots / remainingChannels))
            : 0;
        threadRootBudgets.set(channel, budget);
        remainingCycleThreadRoots = Math.max(0, remainingCycleThreadRoots - budget);
      }
      for (const [channelIndex, channel] of fairScanChannels.entries()) {
        if (remainingCycleMessages <= 0) {
          break;
        }
        const remainingChannels = fairScanChannels.length - channelIndex;
        const channelMessageBudget = Math.max(
          1,
          Math.floor(remainingCycleMessages / remainingChannels),
        );
        const channelState = ensureSlackReconciliationChannelState(state, channel);
        channelState.lastScanAt = nowIso;
        const channelThreadRootBudget = threadRootBudgets.get(channel) ?? 0;
        if (channelThreadRootBudget > 0) {
          channelState.lastThreadScanAt = nowIso;
        }
        ...
        try {
          const result = await collectHistoryMessages({
            ...
            oldest: oldestBound,
            latest: latestBound,
            threadLatest: backlogHighWaterTs,
            maxMessages: channelMessageBudget,
            maxThreadRoots: channelThreadRootBudget,
          ...
          for (const message of messages) {
            ...
            if (status === "missing-admission" && config.autoRecover) {
              const replayAttemptRecorded = await recordSlackAdmission({
                accountId: params.accountId,
                message,
              ...
              if (!replayAttemptRecorded) {
                status = "failed";
                reason = "history-reconcile-attempt-marker-persist-failed";
              ...
              if (shouldStop()) {
                status = "failed";
                reason = "history-reconcile-stopped-before-dispatch";
                ...
                try {
                  ...
                  const delivered = await (
                    params.hasInboundDelivery ?? hasSlackInboundMessageDelivery
                  )({
                    accountId: params.accountId,
                    channelId: message.channel,
                    ts: message.ts,
                  ...
                  if (delivered) {
                    const replayDispatchRecorded = await recordSlackAdmission({
                      accountId: params.accountId,
                      message,
                      source: "history_reconcile",
                    ...
                    status = replayDispatchRecorded ? "replayed" : "failed";
                    reason = replayDispatchRecorded
                      ? "history-reconcile-dispatched"
                      : "history-reconcile-dispatch-marker-persist-failed";
                  } else {
                    status = "failed";
