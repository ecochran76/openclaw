...
export const handleAutomationCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (rest === null) {
    ...
    if (suggestedCommand) {
      if (!params.command.isAuthorizedSender) {
        logVerbose(
          `Ignoring automation suggestion from unauthorized sender: ${params.command.senderId || "<unknown>"}`,
        );
  ...
  if (action === "run") {
    ...
    if (readDetailString(result, "status") !== "accepted") {
      return stopWithText(readDetailString(result, "error") ?? extractAutomationToolText(result));
    }
