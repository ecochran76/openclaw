...
npm_prefix_for_npm_bin() {
  local npm_bin="$1"
  local npm_dir prefix root
  npm_dir="$(dirname "$npm_bin")"
  prefix="$(PATH="$npm_dir:$PATH" "$npm_bin" prefix -g 2>/dev/null || true)"
  prefix="${prefix%/}"
  if [[ -n "$prefix" ]]; then
    printf '%s\n' "$prefix"
    return 0
  fi
  root="$(PATH="$npm_dir:$PATH" "$npm_bin" root -g 2>/dev/null || true)"
  root="${root%/}"
  if [[ "$root" == */lib/node_modules ]]; then
...
install_tarball_globally() {
  local npm_bin="$1"
  local package_dir backup_label backup_tgz backup_metadata npm_prefix
  ...
  if [[ -z "$npm_prefix" && "$package_dir" == */lib/node_modules/openclaw ]]; then
    npm_prefix="${package_dir%/lib/node_modules/openclaw}"
  fi
  if [[ -d "$package_dir" ]]; then
    if [[ -z "$npm_prefix" ]]; then
      echo "error: cannot create a restorable backup without the npm prefix for $package_dir" >&2
      return 1
    fi
    ...
    backup_metadata="$PATCH_WORK_DIR/.openclaw-live-patch-backup-metadata"
    printf 'format=1\nnpm_prefix=%s\npackage_dir=%s\nnpm_bin=%s\n' \
      "$npm_prefix" "$package_dir" "$npm_bin" > "$backup_metadata"
    run tar -czf "$backup_tgz" -C "$(dirname "$package_dir")" "$(basename "$package_dir")" -C "$PATCH_WORK_DIR" "$(basename "$backup_metadata")"
  ...
  if [[ -n "$npm_prefix" ]]; then
    run env NPM_CONFIG_PREFIX="$npm_prefix" "$npm_bin" i -g "$PKG_TGZ"
  else
...
for install_npm_bin in "${INSTALL_NPM_BINS[@]}"; do
  linked_package_dir="$(package_dir_for_npm_bin "$install_npm_bin")"
  if [[ -L "$linked_package_dir" ]]; then
    echo "error: refusing to patch linked global OpenClaw install at $linked_package_dir; the backup would capture only the symlink, not a restorable package tree" >&2
    echo "error: replace the linked install with a packaged global install before retrying" >&2
    exit 1
  fi
done
