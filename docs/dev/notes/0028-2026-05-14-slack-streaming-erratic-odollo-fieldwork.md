...
example tenant Odollo Amazon fulfillment fieldwork exposed another Slack streaming
...
- Channel: `channel-id-redacted` (`#channel-name-redacted` in the operator UI).
- Thread root: `timestamp-redacted`.
- Amazon order: `order-id-redacted`.
- User screenshot reply: `timestamp-redacted`, Slack file
  `file-id-redacted`, filename `screenshot-redacted.jpeg`.

Observed thread sequence:

1. Odollo posted the canonical parent order thread at `timestamp-redacted`.
2. Odollo posted an unnecessary same-state fulfillment status reply at
   `timestamp-redacted`.
3. The user posted a seller-portal screenshot at `timestamp-redacted`.
4. OpenClaw posted progress-only messages:
   - `timestamp-redacted`: `working: reasoning`
   - `timestamp-redacted`: `status: turn appears stalled`
5. OpenClaw posted an interim fallback notice at `timestamp-redacted`:
   `The vision model is unavailable...`
6. OpenClaw posted a useful processing summary at `timestamp-redacted` saying it
   extracted seller-portal fields and updated the sheet.
7. Odollo later posted a deterministic state-advance status at
   `timestamp-redacted`, moving the thread to `tracking pending`.
