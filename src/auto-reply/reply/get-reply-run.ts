...
export async function runPreparedReply(
  ...
  const markStartupProgress = async (
    phase: string,
    meta?: Record<string, string | number | boolean | undefined>,
  ) => {
    await opts?.onStartupProgress?.({ phase, meta });
  };
  await markStartupProgress("prepared reply started");
  ...
  await markStartupProgress("reply context resolved");
  ...
  const bareResetPromptState =
    isBareSessionReset && workspaceDir
      ...
      : null;
  await markStartupProgress("reset prompt resolved");
  const startupContextPrelude =
    ...
    shouldApplyStartupContext({ cfg, action: startupAction })
      ...
      : null;
  await markStartupProgress("startup context resolved");
  ...
  await markStartupProgress("session hints applied");
  ...
  await markStartupProgress("skills resolved");
  ...
  await markStartupProgress("prompt bodies rebuilt");
  const isRoomEvent = inboundEventKind === "room_event";
  if (!resolvedThinkLevel) {
    resolvedThinkLevel = await modelState.resolveDefaultThinkingLevel();
  }
  let thinkingCatalog: Awaited<ReturnType<typeof modelState.resolveThinkingCatalog>> | undefined;
  ...
  await markStartupProgress("thinking resolved");
  if (!thinkingLevelSupported) {
  ...
  await markStartupProgress("session state resolved");
  ...
  await markStartupProgress("auth profile resolved", {
    authProfileId,
    authProfileIdSource,
  });
  ...
  await markStartupProgress("agent runner loaded");
  const queueKey = sessionKey ?? sessionIdFinal;
  preparedSessionState = resolvePreparedSessionState();
  await markStartupProgress("queue state resolving");
  ...
  await markStartupProgress("queue state resolved");
  if (isActive && activeRunQueueAction === "run-now") {
    const queueState = await resolvePreparedReplyQueueState({
      ...
      refreshPreparedState: async () => {
        ...
        preparedSessionState = resolvePreparedSessionState();
        await markStartupProgress("auth profile refreshed", {
          authProfileId,
          authProfileIdSource,
        });
