...
describe("turn tracker", () => {
  it("shares coupled lifecycle state across module reloads", async () => {
    const sessionKey = "agent:main:split-module";
    const startedAt = Date.now();
    const turn = startTrackedTurn({ sessionKey, startedAt, phase: "reasoning" });
    attachTrackedTurnRunId(turn.turnId, "run-split-module");

    vi.resetModules();

    const reloaded = await import("./turn-tracker.js");
    expect(reloaded.getActiveTrackedTurn(sessionKey)).toMatchObject({
      turnId: turn.turnId,
      runId: "run-split-module",
      phase: "reasoning",
    });

    expect(
      reloaded.updateTrackedTurn(turn.turnId, {
        phase: "tool_wait",
        activeTool: "exec",
        markProgress: true,
        at: startedAt + 100,
      ...
      phase: "tool_wait",
      activeTool: "exec",
      lastProgressAt: startedAt + 100,
    });
    expect(getActiveTrackedTurn(sessionKey)).toMatchObject({
      phase: "tool_wait",
      activeTool: "exec",
    });

    expect(
      reloaded.finishTrackedTurn({
        turnId: turn.turnId,
        completedAt: startedAt + 200,
        status: "done",
      }),
    ).toMatchObject({ status: "done", completedAt: startedAt + 200 });
    expect(getActiveTrackedTurn(sessionKey)).toBeUndefined();
    expect(getRecentTrackedTurns(sessionKey)).toMatchObject([
      { turnId: turn.turnId, runId: "run-split-module", status: "done" },
    ]);

    reloaded.resetTrackedTurnsForTests();
    expect(getRecentTrackedTurns(sessionKey)).toEqual([]);
  });

  it("archives a displaced same-session turn without leaking late completion state", () => {
    const base = Date.now();
    ...
    attachTrackedTurnRunId(first.turnId, "run-first");
    ...
    attachTrackedTurnRunId(second.turnId, "run-second");

    expect(getActiveTrackedTurn("agent:main:overlap")?.turnId).toBe(second.turnId);
    expect(getRecentTrackedTurns("agent:main:overlap")).toMatchObject([
      {
        turnId: first.turnId,
        status: "error",
        phase: "error",
    ...
    const lateFirst = finishTrackedTurn({
      turnId: first.turnId,
      completedAt: base + 150,
      status: "done",
    });
    expect(lateFirst).toBeUndefined();
    expect(getActiveTrackedTurn("agent:main:overlap")?.turnId).toBe(second.turnId);

    const finishedSecond = finishTrackedTurn({
      turnId: second.turnId,
      completedAt: base + 200,
      status: "done",
    });

    expect(finishedSecond?.status).toBe("done");
    expect(updateTrackedTurn(first.turnId, { markProgress: true, at: 400 })).toBeUndefined();
    expect(getActiveTrackedTurn("agent:main:overlap")).toBeUndefined();
    expect(getRecentTrackedTurns("agent:main:overlap")).toMatchObject([
      { turnId: second.turnId, status: "done", completedAt: base + 200 },
      {
        turnId: first.turnId,
        status: "error",
        completedAt: base + 100,
  ...
  it("clears a prior delivery error when a later delivery succeeds", () => {
    const sessionKey = "agent:main:delivery-recovery";
    const turn = startTrackedTurn({ sessionKey, startedAt: 0 });
    updateTrackedTurn(turn.turnId, {
      deliveryState: "delivery_failed",
      lastDeliveryError: "transport failed",
      at: 10,
    });
    ...
    expect(getActiveTrackedTurn(sessionKey)).not.toHaveProperty("lastDeliveryError");
  });

  it("bounds recent turn history across distinct sessions", () => {
    for (let index = 0; index <= 500; index += 1) {
      const turn = startTrackedTurn({
        sessionKey: `agent:main:recent-${index}`,
        startedAt: index * 10,
      });
      finishTrackedTurn({ turnId: turn.turnId, completedAt: index * 10 + 1 });
    }

    expect(getRecentTrackedTurns("agent:main:recent-0")).toEqual([]);
    expect(getRecentTrackedTurns("agent:main:recent-1")).toHaveLength(1);
    expect(getRecentTrackedTurns("agent:main:recent-500")).toHaveLength(1);
  });

  it("clears the active tool when a lifecycle update explicitly supplies undefined", () => {
    const turn = startTrackedTurn({
      sessionKey: "agent:main:tool-lifecycle",
      startedAt: 0,
      phase: "tool_wait",
    });
    updateTrackedTurn(turn.turnId, {
    ...
    const updated = updateTrackedTurn(turn.turnId, {
      phase: "reasoning",
    ...
    expect(updated).toMatchObject({ phase: "reasoning", lastProgressAt: 2_000 });
    expect(updated).not.toHaveProperty("activeTool");
    expect(getActiveTrackedTurn("agent:main:tool-lifecycle")).not.toHaveProperty("activeTool");
  });
