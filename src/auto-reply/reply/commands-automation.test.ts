...
describe("/automation command", () => {
  ...
  it("does not suggest automation commands to an unauthorized sender", async () => {
    const params = buildCommandTestParams(
      "Please set up an /automation run that will finish the plan, max 5 turns",
      baseCfg,
    );
    ...
    expect(hoisted.createAutomationToolMock).not.toHaveBeenCalled();
    expect(hoisted.executeMock).not.toHaveBeenCalled();
  });
  ...
  it("surfaces a run tool error instead of claiming the automation started", async () => {
    hoisted.executeMock.mockResolvedValue({
      details: {
        status: "error",
        error: "automation concurrency limit reached (1/1)",
      },
    });

    const params = buildCommandTestParams("/automation run finish the task", baseCfg);
    const result = await handleCommands(params);

    expect(result.shouldContinue).toBe(false);
    expect(result.reply?.text).toBe("automation concurrency limit reached (1/1)");
    expect(result.reply?.text).not.toContain("Automation started");
  });
