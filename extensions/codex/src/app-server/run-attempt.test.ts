...
>[0]["appServer"] {
  return {
    ...
    turnTerminalIdleTimeoutMs: 120_000,
    approvalPolicy: "never",
    approvalsReviewer: "user",
...
describe("runCodexAppServerAttempt", () => {
  ...
  it("releases completion immediately on terminal codex error notifications", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const harness = createStartedThreadHarness();
    ...
    await expect(run).resolves.toMatchObject({
      promptError: "unexpected status 401 Unauthorized",
  ...
  it("fails fast when codex stalls after a ChatGPT auth refresh response", async () => {
    ...
    expect(response).toMatchObject({
      ...
      chatgptAccountId: "eric@example.test",
    });
    await expect(run).resolves.toMatchObject({
      promptErrorSource: "prompt",
      timedOut: true,
