...
export async function runCodexAppServerAttempt(
  ...
  const clearAuthRefreshResponseIdleTimer = () => {
    if (authRefreshResponseIdleTimer) {
      clearTimeout(authRefreshResponseIdleTimer);
      authRefreshResponseIdleTimer = undefined;
    }
  };
  ...
  const fireAuthRefreshResponseIdleTimeout = () => {
    ...
    if (idleMs < authRefreshResponseIdleTimeoutMs) {
      scheduleAuthRefreshResponseIdleWatch();
      return;
    }
    ...
    turnCompletionIdleTimeoutMessage = `Codex app-server did not emit a turn event after refreshing ChatGPT auth tokens; the selected OpenAI Codex auth profile${profileHint} may need re-authentication.`;
    projector?.markTimedOut();
    trajectoryRecorder?.recordEvent("turn.auth_refresh_response_idle_timeout", {
      threadId: thread.threadId,
      turnId,
      idleMs,
    ...
    embeddedAgentLog.warn("codex app-server turn stalled after auth token refresh", {
      threadId: thread.threadId,
      turnId,
      idleMs,
      timeoutMs: authRefreshResponseIdleTimeoutMs,
  ...
  const armAuthRefreshResponseIdleWatch = () => {
    authRefreshResponseWatchArmed = true;
    authRefreshResponseLastActivityAt = Date.now();
    scheduleAuthRefreshResponseIdleWatch();
  };

  const disarmAuthRefreshResponseIdleWatch = () => {
    authRefreshResponseWatchArmed = false;
    clearAuthRefreshResponseIdleTimer();
  };
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    if (isCurrentTurnNotification) {
      disarmAuthRefreshResponseIdleWatch();
    ...
    const isTurnAbortMarker =
      isCurrentTurnNotification &&
      isCodexTurnAbortMarkerNotification(notification, {
        currentPromptTexts: [codexTurnPromptText, promptBuild.prompt],
      });
...
function resolveCodexAuthRefreshResponseIdleTimeoutMs(value: number | undefined): number {
  if (value === undefined) {
