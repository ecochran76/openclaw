...
export async function relayTurn(
  ...
  if (!relayText.trim()) {
    if (policy.requireDelivery) {
      const roles =
        policy.mode === "dual-channel" ? (["source", "target"] as const) : (["target"] as const);
      return {
        status: "blocked",
    ...
    try {
      const response = await gatewayCall({
        ...
        params: {
          ...
          // A turn id distinguishes successive mirrored messages while keeping
          // a retry of this exact relay turn idempotent at the transport.
          idempotencyKey: `${params.runContextId}:relay:${params.turnId}:${params.fromAgent}:${params.toAgent}:${spec.role}:${index}`,
        },
        timeoutMs: 10_000,
      });
      results.push({
