...
export function buildChannelsWhySilentReport(params: {
  ...
  const socketErrorIsCurrent =
    ...
    (latestReceiverAt === undefined || latestReceiverAt <= lastSocketErrorAt);
  const healthState =
  ...
  if (newestAt && (!lastInboundAt || newestAt > lastInboundAt + 1000)) {
    ...
    return {
      ...
      explanation:
        "The channel history contains a newer message than OpenClaw's last inbound timestamp for this account.",
    };
  }
...
export async function channelsWhySilentCommand(
  ...
  deps?: { callGateway?: typeof callGateway; sameGateway?: boolean },
) {
  ...
  const rawStatusPayload = await callGatewayFn({
  ...
  const statusPayload = unwrapGatewayPayload(rawStatusPayload);
  const actionPayload = await callGatewayFn({
  ...
  const sameGateway =
    deps?.sameGateway ??
    (deps?.callGateway ? false : getRuntimeConfig().gateway?.mode !== "remote");
  const admissionRecords =
    channel === "slack" && sameGateway
      ? await readSlackAdmissionRecordsForWhySilent({ accountId })
      : [];
