...
function messageContainsBotMention(
  ...
  const normalizedBotUserIds = new Set(botUserIds.map((id) => id.trim().toUpperCase()));
  for (const match of text.matchAll(/<@([^>|]+)(?:\|[^>]*)?>/g)) {
    if (match[1] && normalizedBotUserIds.has(match[1].trim().toUpperCase())) {
      return true;
    }
  }
  return false;
...
export function scanSlackAdmissionGaps(params: {
  ...
  for (const message of params.messages) {
    ...
    const replayAlreadyDispatched = admission?.outcome === "replay-dispatched";
    ...
    records.push({
      ...
      replayEligible: !replayAlreadyDispatched,
      ...(replayAlreadyDispatched
        ? {
            replayBlockedReason: "watchdog-replay-already-dispatched",
            ledgerRecord: summarizeLedgerRecord(admission),
          }
        : {}),
    });
